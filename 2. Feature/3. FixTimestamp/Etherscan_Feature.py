...
if __name__=='__main__':
    datas = pd.read_csv('FalseData_oneDay2.csv').to_dict('records')
    error_list = []

    #하나의 파일 단위로 Creator Address 불러오고, 해당 초기 유동성풀 이더값 구해온다.
    datas_len = len(datas)
    try:
        p = Pool(1)
        count = 0
    ...
    df.to_csv('FalseData_oneDay4.csv',encoding='utf-8-sig',index=False)
